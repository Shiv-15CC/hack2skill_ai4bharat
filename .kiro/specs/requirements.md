...
The system shall predict salary progression in Indian Rupees (LPA) for each year across all three scenarios.
...
WHEN generating projections, the system shall use Indian market salary data from sources including Glassdoor, Naukri, AmbitionBox, and LinkedIn.

**REQ-SIM-010** [Ubiquitous]  
The system shall display confidence intervals (±X LPA) for each salary prediction.
...
The system shall rank missing skills by impact score (1-100) based on their influence on salary and role attainment.
...
The system shall quantify the impact of specific actions on salary outcomes (e.g., "+2 internships → +12 LPA in Year 5").
...
The system shall explain how resume weaknesses reduce career trajectory probability (e.g., "Lack of system design → -32% backend role probability").

**REQ-RES-019** [Ubiquitous]  
The system shall quantify the salary impact of resume improvements (e.g., "Adding X project → +2 LPA potential").
...
The system shall prioritize resume suggestions by ROI (Return on Investment) - highest salary impact per effort unit.
...
The system shall display salary data in Indian Rupees (LPA) as primary currency.

**REQ-NFR-040** [Localization]  
The system shall use Indian market data sources for all career projections and benchmarks.
...
The system shall cross-reference salary projections against multiple data sources and flag outliers.
...
**CONST-001**  
The system must use Indian market data sources (Naukri, AmbitionBox, Glassdoor India) for salary projections.
...
**ASSUM-003**  
Job market data from public sources (Glassdoor, Naukri) is reasonably accurate for projections.
...
**TEST-002**  
Verify career simulation generates three scenarios (best, average, worst) with salary projections in LPA.
...
**ACCEPT-002**  
System generates 5-year career projection with three scenarios showing salary progression in LPA.
...
**ACCEPT-012**  
Users trust the salary projections as realistic based on Indian market data.
...
**Career Trajectory:** Predicted path of role progression and salary growth over time.

**Impact Score:** Quantified measure (1-100) of how much a skill/action affects career outcomes.

**ROI (Return on Investment):** Salary gain per unit of effort invested in learning or resume improvement.
